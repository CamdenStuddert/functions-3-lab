...
const citiesFiltered = cities.filter(city => city.endsWith(`i`))
...
// const fruitRace = fruits.forEach((element, index) => {
...
const riceDetective = foods.filter(obj => obj.tags.includes(`rice`)).map(obj => obj.price).reduce((acc, cur) => acc + cur)
console.log(riceDetective)
